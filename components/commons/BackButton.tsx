...
const BackButton = ({ href, color }:ButtonTypes) => {
    ...
    return (
        <AnimatedLink 
            href={href}
            className='hidden relative lg:flex md:flex-row-reverse items-center justify-center gap-2 px-5 py-1 overflow-hidden mr-4'
         >
            <div className='hidden relative lg:flex md:flex-row-reverse items-center justify-center gap-2 px-5 py-1 overflow-hidden mr-4' ref={container} onMouseEnter={mouseEnterAnimation} onMouseLeave={mouseLeaveAnimation}>
                ...
                <p style={{color: `${color}`}} className={`back-text opacity-0 -translate-y-[120%] -rotate-12`}>Back</p>
            </div>
        </AnimatedLink>
    );
};
