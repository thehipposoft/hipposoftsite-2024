...
const SingleProyect = ({project}: any) => {
    return (
        <div className='min-h-screen flex flex-col justify-between py-12 relative'>
            ...
            <div className='md:w-[85vw] w-[75vw] flex justify-between items-center mx-auto relative z-10'>
                <h1 className={`text-[${project.nameColor}] text-4xl`}>{project.name}</h1>
                <BackButton href={'/portfolio'} color='#000000'/>
            ...
            <div className='md:w-[85vw] w-[75vw] md:bg-inherit bg-white/70 py-4 md:py-0 rounded-xl flex md:flex-row flex-col justify-between items-center mx-auto relative z-10 gap-2 md:gap-0'>
                <Image src={'/assets/logo.png'} alt='Logo' width={200} height={150} />
                <div className='flex md:flex-row flex-col gap-8'>
                    <div className='flex flex-col'>
                        <h4 className='text-xl text-black'>{project.name}</h4>
                        <p className='uppercase text-sm text-black text-sora pt-2'>{project.industry}</p>
                        <p className='uppercase text-sm text-black text-sora'>{project.work}</p>
                        <div className='flex gap-5 pt-2'>
                            <h4 className='text-2xl text-black'>{project.year}</h4>
                            <Image src={project.flag} alt='country flag' width={35} height={24} />
                            <div className='flex gap-2'>
                                <Image src={project.technologies_icons[0]} alt='wordpress icon' width={25} height={20}  />
                                <Image src={project.technologies_icons[1]} alt='react-icon' width={25} height={20} className={`${project.technologies_icons[1] ? '' : 'hidden'}`}  />
                            </div>
                        </div>
                    </div>
                    <div className='flex justify-center items-center'>
                        <Link href={project.href} className='md:px-6 px-4 tracking-[0.2em] hover:animate-pulse md:text-sm text-xs font-medium md:py-3 py-2 bg-black text-white w-fit rounded-3xl'>
                            DISCOVER SITE
                        </Link>
                    </div>
                </div>
